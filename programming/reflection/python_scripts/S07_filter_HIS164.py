...
# basic settings
cmd.delete("all")
cmd.load("F:/structures/S07/structure.pdb")
cmd.select("CA_S07", "/////CA")
cmd.show("spheres", "CA_S07")
cmd.set("sphere_scale", 0.5, "CA_S07")
cmd.hide("cartoon")

target_ca = "CA_164"
ca_group = "HIS_164"
HIS_data = pd.read_csv("F:/Bristol/courses/DSP/codes/filter/S07_filter_HIS164.csv", index_col=0)
HIS_index = list(HIS_data.index)
HIS_corr = list(HIS_data['HIS_164'])
atom_type = "CA_"
basic_path_pre = "/CA_S07///"
basic_path_sub = "/CA"
cmd.select(target_ca, "/CA_S07///164/CA")
